...
Esta sessão rodou só na nuvem — **não houve conexão com o seu computador** em
nenhum momento da madrugada (a ferramenta de acesso ao PC não estava
disponível). Por isso não dá pra "escrever os arquivos direto em
`C:\Users\andre\Desktop\DSPHub00`" como planejado antes de você dormir.

Em vez disso, o resultado está em **dois arquivos anexados nesta conversa**:

- **`rodada-a.patch`** — um diff único (git) com tudo que mudou desde o commit
  que bate com o seu PC (`baseline: twilio removed`). Já testei aplicando
  esse patch numa cópia limpa exatamente desse commit, e o `tsc` passa limpo
  em cima do resultado.
- **`ENTREGA_RODADA_A.md`** (este arquivo).

Nada foi commitado, dado push ou rodado como migration no seu banco — as
regras da noite continuam valendo.

### Como aplicar (Git Bash, dentro de `DSPHub00`)

```bash
cd /c/Users/andre/Desktop/DSPHub00
git status                     # confirme que está limpo (sem alterações locais)
git apply --check rodada-a.patch   # só verifica, não muda nada
git apply rodada-a.patch           # aplica de verdade
```

O patch não toca em `package.json`, `package-lock.json` nem `.env` — não
precisa rodar `npm install` de novo. Ele **apaga** um arquivo
(`api/src/common/middleware/organization.middleware.ts`, substituído pelo
novo sistema de tenancy) — o `git apply` já faz essa remoção sozinho.

Se `git apply --check` reclamar de fim de linha (CRLF/LF), rode antes:
`git config apply.whitespace nowarn` — é só um aviso, não um erro (eu
testei e o patch aplica limpo).
...
1. **Loop infinito na tela de Disponibilidade dos motoristas**
   ...
   **local do servidor/navegador**, mas convertia pra texto usando **UTC**
   ...
   horário do Reino Unido) — vale testar essa tela depois de aplicar o patch.
...
## Rodar localmente (depois de aplicar o patch)
...
```bash
git apply -R rodada-a.patch     # desfaz o patch por completo
```

Isso reverte todos os arquivos modificados e recria o
`organization.middleware.ts` apagado. Nenhuma migration é desfeita
automaticamente por isso — se você já rodou `prisma migrate dev` e quer
voltar atrás no banco também, me avise que preparamos o rollback certo (não
é seguro eu improvisar isso sem você olhar).
